...
switch ($response) {    //-1
	case "content":
	    if(basename($_SERVER['PHP_SELF'])=="payment.php"){
	        ...
	        $titleTag = '<title>' . @$title . ' - BC2MATCH</title>';
	    }
...
' . $cssInline . '
		...
		if($curPageName=="payment.php"){
			if(isset($_SESSION['usr_id']) && $_SESSION['usr_id']!=0){
				$nContent = $cont;
			} else {
				$nContent = '<div class="login"  style="display:block; align:left;">
				...
				<table cellspacing="5" cellpadding="0" border="0">
					<tbody><tr><td>
						...
						<input type="hidden" name="rtpp" value="payment.php" />
			...
			$templateData = str_replace("<<<content>>>", $nContent, $templateData);
		} else {
