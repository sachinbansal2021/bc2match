...
XJS;
		...
		case "removeUser":
			...
			$content = '<form method="post">
				<table style="display:inline-block;vertical-align:top;">
					<tbody>
						<tr><td colspan="2">Are you sure you want to remove this user from your company profile?</td></tr>
					</tbody>
				</table><br/><br/>
				<input type="hidden" name="op" value="removeDone" /><input type="submit" value="Remove User" />
				</form>
			';
			$response = "ajax";
			require "inc/transmit.php";
		...
		case "removeDone":
		case "passwordUpdate":
		    //echo "SELECT * FROM usr WHERE usr_id = '".CleanI($_REQUEST['usr'])."'- password: ".$_REQUEST['pass']."<br>";
			$ut = Q2R("SELECT * FROM usr WHERE usr_id = '".CleanI($_REQUEST['usr'])."'");
			if (isset($_REQUEST['sendMessage'])) {
					$tmp_content = "\nYour account password has been changed.  \n\nYour new password is:  " . Clean($_REQUEST['pass']) . "\n\n";
					mail($ut['usr_email'],"BC2Match Password Change",$tmp_content,'From: info@bc2match.com' . "\r\n" . 'Reply-To: no-reply@bc2match.com' . "\r\n" . 'X-Mailer: PHP/' . phpversion() );
			}
			//echo "UPDATE usr SET usr_password = '".sha1($_REQUEST['pass'])."' WHERE usr_id ='".CleanI($_REQUEST['usr'])."' "; exit();
