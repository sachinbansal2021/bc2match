...
switch(Clean($_REQUEST['op'])) {
	...
	case "usearch-date":
	    $_SESSION['search_fromdate'] = Clean($_REQUEST['from_date']);
		$_SESSION['search_todate'] = Clean($_REQUEST['to_date']);
		
	
		$searchresults = 1;
		$search_type = "usrlogin";
...
<tbody>';
		...
		$content .= '<tr>
		<td width="100" valign="Top" align="left" bgcolor="'.$cellbgcolor.'"><br><strong>User Logins</strong><br><!-- lloyd4tojo br --></td>		
		<td width="490" valign="Top" align="left" bgcolor="'.$cellbgcolor.'">&nbsp;<br><!-- lloyd 4 tojobr  --></td>
		</tr>';
 
	  	$content .= '<tr>	<td width="100" valign="Top" align="left" bgcolor="'.$cellbgcolor.'">From Date:</td>		
		<td width="490" valign="Top" align="left" bgcolor="'.$cellbgcolor.'"><input type="date" name="from_date" id="first_name"><br></td>
		</tr>';
	 
		$content .= '<tr>
		<td width="100" valign="Top" align="left" bgcolor="'.$cellbgcolor.'">To Date:</td>		
		<td width="490" valign="Top" align="left" bgcolor="'.$cellbgcolor.'"><input type="date" name="to_date" id="last_name"><br></td>
		</tr>';
...
<input type="hidden" name="op" value="usearch-date" />
</form>
</td>
</tr>';
$content .= '</td><td>';
...
$content .= '<table>
        <tr>
        <td><a href="bc2_admins.php">Return to Admin Dashboard</a></td>
        <td></td>
        </tr>
</table>';
...
if (($searchresults == 1) || ($_REQUEST['ptype'] == 'admin')) {
	...
	if ($search_type == "usrlogin")
	{
	    
	    $fd = $_SESSION['search_fromdate'];
		$td = $_SESSION['search_todate'];
	    
	    $fromdate = substr($fd,0,4).substr($fd,5,2).substr($fd,8,2);
		$todate = substr($td,0,4).substr($td,5,2).substr($td,8,2);
		
		
		
		//echo "From: ".$fromdate."<br>To: ".$todate;exit();
	    
	    //SELECT `login_id`, `login_usr_id`, `login_date` FROM `usr_login` WHERE 1
	    //$usr_search = 'select count(*) as total_login, login_id, U.usr_firstname, U.usr_lastname, U.usr_email, U.usr_type, U.usr_phone from usr_login inner join usr U where login_date between '.$fromdate.' and '.$todate;
        $usr_search = "select count(*) as total_logins, login_usr_id, U.usr_firstname, U.usr_lastname, U.usr_email, U.usr_type, U.usr_phone from usr_login ul inner join usr U on ul.login_usr_id = U.usr_id where login_date between '".$fromdate."' and '".$todate."' group by login_usr_id";

	    //echo $usr_search; exit();
	    $firstcolname = "Total Logins";
	    $firstcolw = 100;
	    $lastcolname = "";
	    $lastcolw = 170;
