...
if ( $_SESSION['contactus'] == 1 || $template == "contactus") {
    $dbNav = ''; $dnQ = '';
}
else
{
  switch ($_SESSION['usr_auth']) {
	...
	case "9": $contypes .= "'Employers', 'PubEmp', 'AppEmp', 'EmpMgr'"; break;
        ...
        if  ( $_SESSION['contactus'] == 1) $contypes .= "'contactus'" ;
   ...
   // echo (" $my_pagename" . $my_pagename  . " - " . basename($_SERVER['PHP_SELF']) );
    ...
    $corepos = strpos($my_pagename , $corefindme);
        if  ( $corepos) $contypes = "'NewEmp'";
        echo (" $my_pagename" . $my_pagename  . " -? " . basename($_SERVER['PHP_SELF']) .	", $corefindme: " . $corefindme . ", $corepos: " .  $corepos . "$contypes ="$contypes = .$contypes =);
...
}  // end of check to see ifpagename is contactuf
switch ($response) {
	case "content":
